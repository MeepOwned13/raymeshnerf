...
class LVolume(L.LightningModule):
    ...
    def bkp_locate_density_gradient_based_surface_depth(
        ...
        while iters < max_iters:
            ...
            with torch.no_grad():
                ...
                step_size = (grad * gamma).clamp(
                    -min_step_size * 5 + torch.rand(1, dtype=torch.float32, device=self.device) * min_step_size,
                    min_step_size * 5 - torch.rand(1, dtype=torch.float32, device=self.device) * min_step_size
                )
