...
def suggest_new_eq_angles(c2ws: torch.Tensor, angle_count: int = 4):
    """Suggest new extrinsic matrices with mean distance and FPS

    Utilizes Farthest Point Sampling for angle choices, new angles are generated by equidistance sampling the a sphere
    with radius of mean existing camera distance to origin. 

    Args: 
        c2ws (shape[N, 4, 4]): Extrinisic camera matrices (Camera to World)
        angle_count: How many new angles to suggest

    Returns:
        new_c2ws (shape[angle_count, 4, 4]): Suggested extrinsic camer matrices
    """
    cam_pos = c2ws[:, :3, -1].clone()

    # New cams at mean distance from 0 by existing cams
    radius = torch.mean(torch.sqrt(torch.sum(torch.pow(cam_pos, 2), -1))).item()
    new_c2ws = []
    for phi, theta in zip(*equidistance_rotations(c2ws.shape[0] * 2)):
        new_c2ws.append(look_at(radius, phi, theta))
    new_c2ws = torch.stack(new_c2ws, 0)
    new_cam_pos = new_c2ws[:, :3, -1].clone()
    
    distances, _ = torch.sqrt(torch.sum(torch.pow(new_cam_pos[:, None, :] - cam_pos[None, :, :], 2), -1)).min(dim=1)
    picked = [distances.argmax().item()]

    for _ in range(1, angle_count):
        last = new_cam_pos[picked[-1]]
        distance_to_last = torch.sqrt(torch.sum(torch.pow(new_cam_pos - last, 2), -1))
        distances = torch.minimum(distances, distance_to_last)
        picked.append(torch.argmax(distances).item())

    return new_c2ws[picked]
